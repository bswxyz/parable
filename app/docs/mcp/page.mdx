...
can list and fetch components and templates directly, then run the install
command they get back.
...
Parable is a remote, stateless Streamable-HTTP server — no install, no API key.
Point any MCP-capable client at the endpoint below.

> **Domain note:** the `parable.dev` custom domain is not live yet. Until it is,
> use the Vercel deployment URL: `https://parable-three.vercel.app/api/mcp`.
...
Once connected, ask your agent to "list Parable hero backgrounds" or "install
the shimmer button", and it can browse and pull from the catalog directly.
